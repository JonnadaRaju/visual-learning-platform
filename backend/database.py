...
def get_connection():
    try:
        ...
        raise RuntimeError(f'Failed to connect to PostgreSQL using DATABASE_URL: {exc}') from exc
...
def initialize_database() -> None:
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_simulation_parameters_unique ON simulation_parameters(simulation_id, param_name);
    CREATE INDEX IF NOT EXISTS idx_simulation_runs_session_id ON simulation_runs(session_id);
    """
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
...
def insert_run(session_id: str, simulation_slug: str, input_params: Dict, result_payload: Dict) -> uuid.UUID:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(query, (session_id, simulation_slug, Json(input_params), Json(result_payload)))
            run_id = cursor.fetchone()['id']
        connection.commit()
    return run_id

def mark_run_saved(session_id: str, run_id: uuid.UUID) -> bool:
    query = "UPDATE simulation_runs SET is_saved = TRUE WHERE id = %s AND session_id = %s RETURNING is_saved"
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
...
def upsert_simulation(item: Dict) -> uuid.UUID:
    query = """
        ...
        VALUES (%s, %s, %s, %s, TRUE)
        ON CONFLICT (slug)
        DO UPDATE SET category = EXCLUDED.category, name = EXCLUDED.name, description = EXCLUDED.description, is_active = TRUE
        RETURNING id
    """
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(query, (item['category'], item['name'], item['slug'], item['description']))
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            ...
            for item in parameters:
                cursor.execute(
                    'INSERT INTO simulation_parameters (simulation_id, param_name, param_label, unit, min_value, max_value, default_value, step_size) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',
...
def _rows_to_simulations(rows: List[Dict]) -> List[SimulationDefinition]:
    ...
    for row in rows:
        ...
        if simulation is None:
            simulation = SimulationDefinition(id=row['id'], category=row['category'], name=row['name'], slug=row['slug'], description=row['description'], is_active=row['is_active'], parameters=[])
            simulations[simulation_id] = simulation
        if row.get('parameter_id'):
            simulation.parameters.append(SimulationParameter(id=row['parameter_id'], simulation_id=simulation_id, param_name=row['param_name'], param_label=row['param_label'], unit=row['unit'], min_value=float(row['min_value']), max_value=float(row['max_value']), default_value=float(row['default_value']), step_size=float(row['step_size'])))
